...
async function consumerExample() {
  ...
  const consumer = await createConsumer(config, ({key, value, partition, offset}, ) => {
    const createPaymentsRecord = createPayments(key, value, config);
    console.log(`Processed record ${key} with result ${createPaymentsRecord} in partition ${partition} @ offset ${offset}.`);
  });
